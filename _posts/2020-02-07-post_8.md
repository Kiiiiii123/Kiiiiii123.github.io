...
#### 2. We first simplify our notation a little bit by defining the bellman backup operator B. The operator B operates on a value function (it's a table right now and we can see it as a vector). We represent value functions as vectors and represent rewards for particular actions as vectors. Then we can see an expression in terms of linear algebra. We evaluate this for every possible action, get a bunch of vectors for every action and then we take an element-wise max. One of the things that we can show is that the optimal value function V star is a fixed point of B, it means that when we put the operator to the vector, we get back the same vector. V star is also the value function for the optimal policy. We know that this operator has a fixed point which is the optimal value function, now the real question is does it converge that fixed point? The fixed point always exists, it's always unique and corresponds to the optimal policy. We haven't proven that.
<p align= "center">
<img src="/images/139.png"><br/>
</p>

#### 3. Will we reach the fixed point? We can prove that value iteration reaches V star because B is a contraction, it means that for any V and V bar (two vectors), the difference between the vectors we get by applying B to both of them is less than or equal to gamma times the difference between V and V bar. That is after applying B to V and V bar, they get closer to each other, hence contraction. Something important to note here is that the difference here is quantified in terms of the L-infinity norm ("max" norm). If the factor gamma is one, there's no contraction any more, but for any factor value less than one, the vectors get closer together. We can actually immediately recover a proof of convergence for value iteration. We simply use the fact that V star is a fixed point of B. We choose V bar to be the V star, then BV star is equal to V star, and we substitute in V star for V bar in this euqation. From the definition of contraction and the knowledge that V star is a fixed point of B, we recover the equation at the bottom. What that implies is that if we apply the operator B to any vector V, it will become closer to V star in terms of the L-infinity norm. From this we can actually get a proof of convergence.
<p align= "center">
<img src="/images/140.png"><br/>
</p>

### (19) Non-tabular value function learning
#### 1. We want not have to store a full vector of values in every single state, we want to use some find of function approximator instead, so we get our fitted value iteration algorithm. The argmin is used as an operator that finds a value function in some set of value functions Ω that minimizes the difference between the value function we're finding and the back-up version of the previous value function (training data, old value function V). Ω might be all the value functions represenred by our neural network. We can define a new operator Π, which is just the argmin operator. It's also a projection operator because we're projecting BV to the manifold Ω to get the cloest V'. Π is also a contraction with respect to L2 norm.
<p align= "center">
<img src="/images/141.png"><br/>
</p>

#### 2. So far we have greatly shortened our equation, but we haven't reached any conclusion. The contractions are in different norms L-infinity and L2 and ΠB is not a contraction of any kind. Fitted value iteration might not converge, it might actually move us further away from optimal value function. Then we can get some conclusions.
<p align= "center">
<img src="/images/142.png"><br/>
</p>

### (20) What about fitted Q-iteration?
#### The same exact thing applies to fitted Q-iteration, which means that on-line Q-learning doesn't converge. We need to learn how we can actually implement practical and effective deep learning algorithms.
<p align= "center">
<img src="/images/143.png"><br/>
</p>

### (21) But… it’s just regression!
<p align= "center">
<img src="/images/144.png"><br/>
</p>
  
### (22) A sad corollary
<p align= "center">
<img src="/images/145.png"><br/>
</p>

### (23) Review3
<p align= "center">
<img src="/images/146.png"><br/>
</p>
